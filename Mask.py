...
for i,cnt in enumerate(sorted_contours):
    ...
    area = h * w
    # # if area is less than 300 factor  pixels skip
    #if y/img_h < 0.6 : continue
    if area < 10*30: continue
    if x/(y+0.0001) <  1/4 : continue
    if y < 0.6 *img_h : continue
    ...
    try:
        ...
        cv2.waitKey(0)
        id_print = cv2.putText(img, text[:4], (x, y), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (0, 255, 0), 2)
        cv2.imshow("Id_Print", id_print)
        cv2.waitKey(0)
    except:
        print('pytesseract fails ')
        text = None
img_mask = 'img1_mask_w_id.jpeg'
